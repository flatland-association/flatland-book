...
All the parameters for the lines and timetables in these levels are set by hand. These include the sequence of stations, the time windows at stops (earliest departure and latest arrival) and the frequencies, i.e. the number of steps between the initial earliest departures of trains on a given line. Likewise, the parametrization of the malfunctions is done by hand, however, they are pseudorandom with a given seed to ensure fairness between different submissions. 

To generate environments for training or use the provided example curriculum, see [training_envs.ipynb](training_envs.ipynb).