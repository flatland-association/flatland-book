...
| `WAIT_FOR_POD_TO_START_LIMIT`               | `120` (2 min)                                                                            | Failure                        | submission pod should be listed by now, i.e. pulling has started by now. Unrelated to image size - only covers Job/Pod-object propagation latency in the k8s API.                                                                      |
| `WAIT_FOR_POD_TO_RUN_LIMIT`                 | `1800` (30 min)                                                                          | Failure                        | submission pod should have reached running state by now, i.e. pulling should be done by now. Scales with image size; capped by `active_deadline_seconds - RUNNING_TIME_LIMIT` so a full running-time window still fits after the pull. |
| `RUNNING_TIME_LIMIT`                        | `1800`  (30 min)                                                                         | Success with termination cause | per scenario; evaluation terminated; results do not include the overlong scenario                                                                                                                                                      |
...
| `ACTIVE_DEADLINE_SECONDS`                   | `3600` (1h)                                                                              | Failure/cleanup                | per scenario; everything including technical overhead for starting the submission pod                                                                                                                                                  | 
...
| `ORCHESTRATION_JOB_K8S_RESOURCE_ALLOCATION` | `{"requests": {"memory": "5Gi", "cpu": "1"}, "limits": {"memory": "5Gi", "cpu": "1"}}`   | Failure                        | resource limits for the orchestration pod driving the evaluation (not the pod running the submission itself)                                                                                                                           |
| `K8S_RESOURCE_ALLOCATION`                   | `{"requests": {"memory": "15Gi", "cpu": "4"}, "limits": {"memory": "15Gi", "cpu": "4"}}` | Failure                        | resource limits for the pod running the submission                                                                                                                                                                                     | 
| `ORCHESTRATION_JOB_ACTIVE_DEADLINE_SECONDS` | `28800` (8h)                                                                             | Failure/cleanup                | all scenarios; everything including technical overhead for starting pods for orchestration and evaluation                                                                                                                              | 
